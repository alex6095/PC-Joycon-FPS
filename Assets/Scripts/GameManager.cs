...
public class GameManager : MonoBehaviourPunCallbacks
{
    ...
    void CreatePlayer()
    {
        ...
        GameObject myPlayerGO = PhotonNetwork.Instantiate("Player",
                                  points[idx].position,
                                  points[idx].rotation,
                                  0); 


        myPlayerGO.GetComponent<Fire>().enabled = true;
        myPlayerGO.GetComponent<Movement>().enabled = true;

       
        myPlayerGO.transform.Find("PlayerCamera").gameObject.SetActive(true);
